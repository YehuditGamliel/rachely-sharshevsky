...
export class EnterService {
    ...
    async Authentication(data) {
        ...
        if (verificationResult) {
            const token = jwt.sign({ username: data.userName }, "privateKey", { expiresIn: '90m' });
    ...
    createUser = async (itemDetailes) => {
        try {
            ...
            const token = jwt.sign({ username: newUser.userName }, "privateKey", { expiresIn: '90m' });
            return { newUser: newUser, otpGenerated: otpGenerated, token: token }
        }
        catch (ex) {
            throw ex;
        }
